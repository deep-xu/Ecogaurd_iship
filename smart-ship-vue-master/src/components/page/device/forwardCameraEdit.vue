  ...
  <div>
    ...
    >
      ...
      <div v-if="formData.type == 'camera' && formData.protocol == 'onvif'">
        ...
        <el-form-item label='摄像头账号（请不要包含 " / \ + : 等特殊字符）' prop="username">
          <el-input
            v-if="!onvifInfo.Manufacturer"
            clearable
        ...
        <el-form-item label='摄像头密码（请不要包含 " / \ + : 等特殊字符）' prop="password">
          <el-input
            v-if="!onvifInfo.Manufacturer"
            show-password
            type="password"
      ...
      >
        ...
        >
          <el-input v-model="formData.stream"></el-input>
        </el-form-item>
        <el-form-item
          label="检测是否在线"
          prop="width"
          :rules="{ required: true, validator: validateEncoding, trigger: 'blur' }"
        >
          <div style="display: flex">
            <el-button
              type="primary"
      ...
      <div v-else>
        ...
        >
          <div style="position: absolute; top: -42rem; left: 125rem">
...
import { sourceApis, forwardSourceApis } from "@/api/device.js";
...
export default {
  ...
  methods: {
    ...
    validateEncoding(rule, value, callback) {
      if (this.formData.encoding) {
        callback();
      } else {
        let msg = "请检测视频流状态";
        if (this.formData.type == "camera") {
        ...
        .catch((err) => {
          ...
          if(err.error_code == -1){
            this.$message({
              ...
              showClose: true,
              type: "error",
              message: err.data ? err.data.replaceAll('\n','<br/>') : err.message[this.lang],
            });
          }
