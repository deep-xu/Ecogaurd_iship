<template>
  ...
  <script>
import { openlibApis } from "@/api/libs.js";
      ...
      openlibApis
        ...
        .catch((err) => {
          ...
          if (err.error_code == -1) {
            ...
            this.$emit("update-openlib-table");
          } else {
            this.$message({
              type: "error",
    ...
    handleDownload(type) {
      if (type == "template") {
        openlibApis
          ...
          .then((res) => {
            this.download(res, "openlib_template");
          })
          .catch((err) => {
        ...
        openlibApis
          ...
          .then((res) => {
            ...
            this.$emit("close-importOpenlib");
            this.download(res, `openlib_${this.$props.group_id}`);
            this.$message({
              type: "success",
              message: '下载成功',
            });
