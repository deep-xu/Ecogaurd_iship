...
import { group, ppeApis } from "@/api/libs.js";
import addPpe from "./addPpe.vue";
import importppe from "./importppe.vue";
import { projConfig } from "@/utils/config.js";
import ppeGroupSetting from "./ppeGroupSetting.vue";
export default {
  components: { addPpe, importppe, ppeGroupSetting },
  props: ["category"],
  data() {
    return {
  ...
  methods: {
    ...
    getGroupList() {
      this.groupList = [];
      group
        .getGroupList({ alg: "ppe", device_id: this.search.device_id })
        .then((res) => {
    ...
    getTableData() {
      ...
      ppeApis.getTableData(params).then((res) => {
