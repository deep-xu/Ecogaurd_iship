<template>
  <div class="adduser-page">
    <h3>{{ formDataInit.optype == "add" ? "添加工服" : "编辑工服" }}</h3>
    <el-form
      ref="formData"
      :model="formData"
      :rules="formRule"
    ...
    >
      ...
      <!-- <el-form-item label="忽略图片质量" prop="force" v-if="formData.file">
        <el-switch
        ...
        if (valid) {
          ...
          if (this.$props.formDataInit.optype == "add") {
            ...
            if (this.formData.external_id) {
              params.external_id = this.formData.external_id;
            }
            ...
            if (
              ...
              let params = {
                ...
                external_id: this.formData.external_id || null,
                name: this.formData.name,
                desc: this.formData.desc || null,
                device_ids: [this.formDataInit.device_id],
                group_id: this.$props.group_id
      ...
      workclothesApis
        ...
        .then((res) => {
          if (this.formData.file) {
            ...
            this.$emit("close-add-workclothes");
            let message =
              this.$props.formDataInit.optype == "add"
                ? "新增工服成功"
                : "更新工服成功";
            this.$message({
              type: "success",
        ...
        .catch((err) => {
          ...
          this.$message({
            type: "error",
            message:
              err.data[this.$props.formDataInit.device_id].reason[this.lang],
          });
        });
    },
    saveImg(id) {
