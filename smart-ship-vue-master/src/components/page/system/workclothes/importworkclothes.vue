<template>
  <div class="importlib">
    ...
    <div v-if="type == 'import'">
      <div class="step-download">
        ...
        >
        <ul>
          <li>1.点击下载系统提供的工服信息和照片模板</li>
          <li>2.模板解压，获取工服信息Excel、成员照片文件夹</li>
          <li>3.规范填写Excel工服信息，文件夹中添加已命名注册照</li>
          <li>4.注册照片支持jpg,jpeg,png,bmp格式，命名不可包含中文</li>
          <li>
            5.添加完毕，确认目录层级与模板保持一致，将模板重新压缩为.zip格式
          </li>
        </ul>
      </div>
      <div class="step-upload">
        ...
        >
          <label style="margin-right: 20px;font-size:14rem;">忽略图片质量</label>
  ...
  <script>
import { workclothesApis } from "@/api/libs.js";
...
export default {
  ...
  methods: {
    handleSave() {
      ...
      workclothesApis
        ...
        .then((res) => {
          ...
          this.$emit("close-importworkclothes");
          this.$message({
            type: "success",
        ...
        .catch((err) => {
          ...
          if (err.error_code == -1) {
            ...
            this.$emit("update-workclothes-table");
          } else {
            this.$message({
              type: "error",
    ...
    handleDownload(type) {
      if (type == "template") {
        workclothesApis
          .export({ template: true, device_id: this.$props.deviceid ,group_id: this.$props.group_id})
          .then((res) => {
            this.download(res, "workclothes_template");
          })
          .catch((err) => {
        ...
        workclothesApis
          .export({ device_id: this.$props.deviceid,group_id: this.$props.group_id })
          .then((res) => {
            this.exportloading = false;
            this.$emit("close-importworkclothes");
            this.download(res, `workclothes_${this.$props.group_id}`);
            this.$message({
              type: "success",
              message: '下载成功',
            });
