  ...
  <div class="facelib">
      ...
      >
        ...
        <el-table-column label="Picture">
          <template slot-scope="scope">
            <img :src="scope.row.pic" style="width: 100px" alt="" @click="handleShowImg(scope.row)"/>
          </template>
        </el-table-column>
    ...
    >
      <importworkclothes
        ...
        :group_id="search.group_id"
        @close-importworkclothes="
          getTableData();
          showImport = false;
        "
        @update-workclothes-table="getTableData()"
      ></importworkclothes>
    </el-drawer>
    <el-dialog
      ...
      class="groupsetting-dialog"
      v-dialogDrag
    >
      <workclothesGroupSetting
        v-if="showGroupSetting"
        :device_id="search.device_id"
        :group_id="search.group_id"
        :groupList="groupList"
        @update-workclothes-group="getGroupList"
        @close-workclothes-group="
          getGroupList();
    ...
    >
      <div class="content_box">
        <img :src="imageDetail.pic" style="margin:0 auto;display:block;max-height: 60vh;max-width:100%;"/>
      </div>
    </el-dialog>
  </div>
</template>
...
import { group, workclothesApis } from "@/api/libs.js";
import addWorkclothes from "./addWorkclothes.vue";
import { projConfig } from "@/utils/config.js";
import Importworkclothes from "./importworkclothes.vue";
import workclothesGroupSetting from "./workclothesGroupSetting.vue";
export default {
  components: { addWorkclothes, Importworkclothes, workclothesGroupSetting },
  data() {
    return {
  ...
  methods: {
    ...
    getGroupList() {
      this.groupList = [];
      group
        .getGroupList({ alg: "work_clothes", device_id: this.search.device_id })
        .then((res) => {
    ...
    getTableData() {
      ...
      workclothesApis.getTableData(params).then((res) => {
        this.tableData = res.data.map((item) => {
          item.device =
            this.devices[item.device_id].name ||
            this.devices[item.device_id].id;
          if (item.image) {
            item.pic = projConfig.picprev + item.image;
          }
          return item;
        });
