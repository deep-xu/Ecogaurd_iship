...
export default {
  ...
  methods: {
      ...
      if (this.$props.group_id) {
        ...
        for (let i = 0; i < this.$props.groupList.length; i++) {
          if (this.$props.groupList[i].id == this.$props.group_id) {
            ...
            this.formData.quality = this.$props.groupList[i].ext.quality;
            break;
          }
        }
      } else {
    ...
    handleGroupChg(item, index) {
      ...
      this.formData.quality = item.ext.quality;
    },
    handleAddGroup() {
      this.formData.optype = "add";
      this.formData.id = "";
...
<style lang="less">
.white-theme-container .face-win {
  .groupNameList {
    li .groupname {
      color: #333 !important;
