<template>
  ...
  >
    ...
    >
      ...
      <div class="location-container" v-if="playingCameraList[key]">
        ...
        <div v-if="videosInfos[key] && videosInfos[key].deviceType == 'normal'" style="display: flex; align-items: center">
          ...
          <div class="alg-sel">
            ...
            <div v-else class="algname" style="color: gray">Select Algorithm</div>
            <ul>
              <li
      ...
      <div v-if="item && item.playerState !== 'success'"
           ...
           pointer-events: none;">
        <i class="el-icon-video-camera" style="font-size: 50rem"></i>
...
export default {
      ...
      handler(newdata, olddata) {
        ...
        if (pos != -1) {
          ...
          if (newdata[pos] && newdata[pos] != olddata[pos]) {
            this.setOrisize(false, false, pos);
  ...
  methods: {
    ...
    subscribeLive(cameraId, index) {
      const [sourceId, deviceId] = cameraId.split('__')
      deviceV2Apis
        .getStream({
          deviceId,
          sourceId,
    ...
    setAlarms(data, index) {
      ...
      let bbox = data.reserved_data.bbox;
      ...
      if (bbox.rectangles.length > 0) {
        bbox.rectangles.forEach((item, i) => {
          ...
          let context = this.videosInfos[index].canvas.getContext("2d");
          // 如果算法是person_attribute或vehicle_attribute时，文字块放右侧
          if (
            data.alg.name == "vehicle_attribute" ||
            data.alg.name == "person_attribute"
          ) {
            if (item.ext) {
              let extAttributes = item.ext.attribute;
              let maxWidth = Math.max(
                ...extAttributes.map((item) => context.measureText(item).width)
              );
              ...
              for (let i = 0; i < extAttributes.length; i++) {
                context.fillText(
                  extAttributes[i],
                  coordinates.x1 + 5,
                  coordinates.y + 12 * (i + 1),
                  maxWidth.width
                );
              }
    ...
    drawCountingInfo(context, lines) {
      lines.forEach((item, index) => {
        ...
        if(item.ext && item.ext.direction){
    ...
    mqttSubscribe() {
      if (this.$props.mqttclient) {
        ...
        this.$props.mqttclient.on("message", (topic, payload) => {
          ...
          if (msg.msg_type == "result" && this.$route.path == "/home") {
